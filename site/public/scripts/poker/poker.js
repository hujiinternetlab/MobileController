...
function startPoker() {
	...
	MCClient.openRoom(con, jo, discon, numOfPlayers);
	$("#settingDiv").hide();
...
// Disconnection handler that shows a message when a disconnection from the server occurs.
function discon(roomID) {
...
function con(roomID) {
	...
	console.debug(table); // table debug
	for (var i = 0; i < numOfPlayers; ++i) {
...
function recvTicketEventFunc(uuid) {
	if (setupTickets !== -1) {
...
function disconnectEvent(userID) {
...
function msgEventFunc(inputEvent) {
