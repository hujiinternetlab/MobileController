/*
This code is part of a node-poker project made by Michal Bell.

"author": "Michael Bell",
"name": "node-poker",
"description": "A complete game engine for Texas Holdem poker",
"version": "0.0.2",
"homepage": "https://github.com/mjhbell/node-poker",
"repository": {
"type": "git",
"url": "git://github.com/mjhbell/node-poker.git"
*/

// This function gets the player's 7 card hand and checks all the combinations for
// the best hand possible. It returns a result object with the best hand cards,
// the best rank and the hand's type.
function getBestRankAndHand(hand) {
	var handCombinations = [[0,1,2,3,4], [0,1,2,3,5], [0,1,2,3,6], [0,1,2,4,5], [0,1,2,4,6], [0,1,2,5,6], [0,1,3,4,5], [0,1,3,4,6], [0,1,3,5,6], [0,1,4,5,6], [0,2,3,4,5], [0,2,3,4,6], [0,2,3,5,6], [0,2,4,5,6], [0,3,4,5,6], [1,2,3,4,5], [1,2,3,4,6], [1,2,3,5,6], [1,2,4,5,6], [1,3,4,5,6], [2,3,4,5,6]];
	var bestResult = {};
	bestResult.rank = -1;
	for(var i = 0; i < 21; ++i) {
		var checkHand = {};
		checkHand.cards = new Array(5);
		for(var j = 0; j < 5; ++j) {
			checkHand.cards[j] = hand[handCombinations[i][j]];
		}
		var result = RankHandInt(checkHand);
		if(result.rank > bestResult.rank) {
			bestResult = result;
			bestResult.hand = checkHand.cards;
		}
	}
	return bestResult;
}

// This function gets 5 card hand and returns an object with the rank of the hand
// and the hand's type
function RankHandInt(hand){
	...
	if(rank == 0){
		...
		if(rank > 292){message = 'Straight Flush'};
	};
...
// This function fills the deck and shuffles the cards
function fillDeck(){
	var deck = [];
	deck.push('AS');
	deck.push('KS');
